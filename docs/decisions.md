...
`통보` = 필드 추가라 합의가 필요 없다(`Principles §7.1`). 요청 본문은 아래 §3.1.
...
| 5 | 엔드포인트 **× 업무구분**별 타임아웃과 리스 시간의 부등식 | **요청 R3** | Principles §4.2 · §3.1 |
...
| 9 | ~~**`bsnsDivNm` 을 `item-summary`·`bid-summary` 입력에 실을 것**~~ | **요청 R1 — 재검토** | §3.1. 두 엔드포인트가 폐기됐다 |
| 10 | 첨부 선택 규칙을 업무구분별로 — `/규격서/` 파일명 게이트는 용역·공사에서 빈 결과를 낸다 | **요청 R2** | §3.1 |


### 3.1 요청 5건 · 통보 2건 — 백엔드에 보내는 본문

> 2026-08-28 에 `docs/plan.md` 를 폐기하면서 그 §8 에서 옮겨 왔다. 원본 §8 의 나머지
> (M0.5~M0.7 진행 계획, `gh api` 로 백엔드 코드를 읽는 절차)는 전부 해소돼서 버렸다.

**요청 — 백엔드 코드가 바뀌어야 하는 것**
...
| **R1** | ~~`bsnsDivNm` 을 `item-summary`·`bid-summary` 입력에 실어 달라~~ | — | — | **재검토 필요.** 두 엔드포인트는 2026-08-26 에 `notice-summary` 하나로 합쳐졌고, 그 요청 스키마에 `bsnsDivNm` 이 없다. 업무구분 분기 자체를 되살릴 것인지 먼저 정해야 한다 |
| **R2** | 첨부 선택 규칙을 업무구분별로 — `/규격서/` 파일명 하드 게이트는 용역·공사에서 빈 결과를 낸다 | 첨부 파싱은 백엔드 소유(`CLAUDE.md §4`) | `documents[]` 에 온 것만으로 판정 | 용역·공사 공고를 제대로 요약해야 할 때 |
| **R3** | 엔드포인트별 타임아웃·리스 시간 | 리스는 백엔드 큐 소유. 부등식이 한쪽만으로 성립하지 않는다 | 자체 데드라인만 건다(`CLAUDE.md §2-7`) | 워커 중복 집행으로 LLM 비용이 두 배로 날 때 |
| **R4** | `degraded: true` 를 **성공으로 캐시하지 말 것**(재시도 대상으로 둘 것) | 캐시·재시도 장부가 백엔드 소유 | `failure-modes.md §4` 대로 내보내되 채택은 백엔드에 달렸다 | 부분 결과가 캐시에 눌러앉을 때 |

**통보 — 허락을 구하지 않는다.** 필드 추가는 합의 없이 자유다(`Principles §7.1`).
...
**형식 — 산문이 아니라 패치.** `ai-boundary.md` 는 백엔드 저장소 소유이자 최상위 권위다.
요청의 올바른 형태는 위시리스트가 아니라 **그 문서에 대한 diff** 다. 각 항목에 세 줄을 붙인다 —
`현재 우회` · `답이 오면` · `막히는 때`. 우회를 적으면 우리는 답을 기다리며 멈추지 않고,
